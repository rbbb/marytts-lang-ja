    ...
    public boolean process(Element t, final List<Element> expanded) {
        ...
        if (allowMultipleTokens()) {
            ...
            while (n != null 
                    ...
                    && !n.hasAttribute("ph") 
                    //&& !n.hasAttribute("sounds_like") every token has a sounds like attribute
                    && isCandidate(n)) {
...
//                System.out.println(sb.toString()+" "+matchedType);
            ...
            if (!t.hasAttribute("ph") 
            		//&& !t.hasAttribute("sounds_like") every token has a sounds_like attribute currently from tokeniser 
            		&& isCandidate(t)) {
                sb.setLength(0);
