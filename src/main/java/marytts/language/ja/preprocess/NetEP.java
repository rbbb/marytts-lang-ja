...
public class NetEP extends ExpansionPattern
{
    ...
    /* Email syntax is specified in http://www.faqs.org/rfcs/rfc2822.html
       (see end of this file for excerpt)
    */
    ...
    */
    ...
    protected int match(String s, int type)
    {
        switch (type) {
        case 0:
            if (matchNetEmail(s)) return 1;
            if (matchNetUri(s)) return 2;
            break;
        case 1:
            if (matchNetEmail(s)) return 1;
            break;
    ...
    protected List<Element> expandNetEmail(Document doc, String s)
    {
        ...
        String localPartExpanded =
            localPart; // true = sayPuncutation
        exp.addAll(makeNewTokens(doc,
                                 localPartExpanded,
                                 true, // create mtu
                                 localPart,
                                 true)); // force accents
    ...
    private List<Element> expandDomain(Document doc, String domain)
    {
        ...
        if (domain.endsWith(".com")) {
            ...
            domainSuffix =
                " " + domain.substring(domain.length()-2, domain.length()-1) +
