...
public class TelephoneEP extends ExpansionPattern
{
    ...
    protected final String sMandatoryChars = "[0-9]";
    ...
    private final Pattern reMandatoryChars = Pattern.compile(sMandatoryChars);
    ...
    {
        return reTelephone.matcher(s).matches() && reMandatoryChars.matcher(s).find(); //at least some digits, otherwise '+/-/' is a valid phone number 
    }

    protected int canDealWith(String input, int typeCode)
    {
        if (typeCode != 0) return -1;
    ...
    /**
     * This method, differently from what is usually done,
     * does not take a string argument, but the original tokens.
     * The reason is that grouping of telephone number parts is
    ...
    protected List<Element> expandTelephone(Document doc, List<Element> tokens)
    {
        ...
        if (firstText != null && firstText.length() > 0 &&
            firstText.charAt(0) == '+') {
            exp.addAll(makeNewTokens(doc, "プラス"));
        ...
        //digits are not separated in Korean because of the 1-2 rule
        ...
        exp.addAll(number.expandDigits(doc, phoneNumber, true));
