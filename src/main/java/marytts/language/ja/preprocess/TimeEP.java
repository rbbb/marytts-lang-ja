...
public class TimeEP extends ExpansionPattern
{
    ...
    protected final String sFinal = "(?:h?)";//native time date is using the date/time pattern
    ...
    protected List<Element> expandTimeHMS(Document doc, String s)
    {
		...
		sb.append(listToString(JapaneseNumberToWords.toSinoJapaneseNumber(hour, CounterWord.Hour)));
        
		String minute = reMatcher.group(2);
        if (!minute.equals("00")) {
            sb.append(" ");
            sb.append(listToString(JapaneseNumberToWords.toSinoJapaneseNumber(minute, CounterWord.Minute)));
        }
        ...
        if (!second.equals("00")) {
        	sb.append(" ");
            sb.append(listToString(JapaneseNumberToWords.toSinoJapaneseNumber(second, CounterWord.Second)));
        }
        return exp;
    }
    ...
    protected List<Element> expandTimeH(Document doc, String s)
    {
        ...
        reMatcher.find();
        String hour = reMatcher.group(1); // first bracket pair in reHour: hour
        exp.addAll(makeNewTokens(doc, listToString(JapaneseNumberToWords.toSinoJapaneseNumber(hour, CounterWord.Hour)), true, s));
