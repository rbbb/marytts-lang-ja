...
import marytts.language.ko.lib.KoreanNumbersToWords.CounterWord;
...
public class NumberEP extends ExpansionPattern
{
    ...
    {
    	tempCounterWords = "";
    	for(CounterWord counterWord:KoreanNumbersToWords.CounterWord.values())
    	{
    		if(tempCounterWords.length()==0)
    		{
    			tempCounterWords += "(?:"+counterWord.getHangul();
    		}else{
    			tempCounterWords += "|"+counterWord.getHangul();
    		}
    		tempCounterWordCharacters += counterWord.getHangul();
...
//    protected static final String sFloat = "(?:-?(?:[1-9][0-9]{0,8}|0)?(?:\\.)[0-9]+)";
    ...
    {        
        ...
        List<String> integerWords = KoreanNumbersToWords.toWords(integer, counter);
        String expanded = "";
    ...
    {
    	List<String> integerWords = KoreanNumbersToWords.toWords(s, null);
        String expanded = "";
        for(String integerWord:integerWords)
        {
        	expanded += integerWord+" ";
    ...
    {
    	List<String> integerWords = KoreanNumbersToWords.toSpokenFloat(number);
        String expanded = "";
        for(String integerWord:integerWords)
        {
        	expanded += integerWord+" ";
    ...
    {
    	List<String> integerWords = KoreanNumbersToWords.toSpelledOutNumber(digits.replaceAll("[\\.,]", ""));
        String expanded = "";
        for(String integerWord:integerWords)
        {
        	expanded += integerWord+" ";
